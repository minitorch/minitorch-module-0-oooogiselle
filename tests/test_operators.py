...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert_close(sigmoid(0.0), 0.5) # the sigmoid function crosses 0.5 at x = 0
...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    """
    # TODO: Implement for Task 0.2.
    assert_close(mul(c, add(a, b)), add(mul(c, a), mul(c, b))) # bc sometimes == isn't exact due to floating point precision issues
    #raise NotImplementedError("Need to implement for Task 0.2")
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    listsum = sum(ls1) + sum(ls2)
    elementsum = sum(addLists(ls1, ls2))
    assert_close(listsum, elementsum)
